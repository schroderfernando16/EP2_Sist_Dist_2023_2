...
class Server:
    ...
    def replicate_data(self, message):
        for server in [s for s in self.servers if s != (self.ip, self.port)]:
        ...
        while True:
            ...
            if message.command == "PUT":
                ...
                if message.key in self.data:
                    value, timestamp = self.data[message.key]
                    if float(timestamp) >= float(message.timestamp):
                        response = f"{value} {timestamp}"
                    else:
