...
class Server:
    ...
    def put(self, key, value):
        server_ip, server_port = random.choice(self.servers)  # Movido para o início da função
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        message = Message("PUT", key, value, timestamp)
        response = self.send_message(message, server_ip, server_port)

        if response.startswith("PUT_OK"):
            # Atualizar o timestamp associado à key com o timestamp recebido do servidor
